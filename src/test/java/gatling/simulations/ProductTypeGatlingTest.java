 ...
 */
public class ProductTypeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all productTypes").get("/api/product-types").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new productType")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created productType").get("${new_productType_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created productType").delete("${new_productType_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ProductType entity").exec(scn);
